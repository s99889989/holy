...
// power=3 讓近的點影響更大，遠的點影響快速衰減
function gpsToImgFraction(lat, lng) {
  const power = 3
  let sumW = 0, sumX = 0, sumY = 0
  for (const cp of CONTROL_POINTS) {
    const dLat = (lat - cp.lat) * 111000
    const dLng = (lng - cp.lng) * 102000
    const dist2 = dLat * dLat + dLng * dLng
    if (dist2 < 0.0001) return {x: cp.imgX, y: cp.imgY} // 幾乎在控制點上
...
const landmarks = ref([
  ...
  },
  {
    ...
    imgX: 0.400,
    imgY: 0.680,
    desc: '核心示範區，含樂智團體家屋、廣場、農藝區、小規模多機能。',
    tag: 'orange',
    tagLabel: '重點參觀'
  },
  {
    ...
    imgX: 0.620,
    imgY: 0.711,
    desc: '1F 用餐空間，主打健康有機食材，適合安排團體午餐，需提前訂位。',
    tag: 'gold',
    tagLabel: '餐飲'
  },
  {
  ...
  <div class="overflow">
    ...
    <section>
      <div class="cover-box">
        ...
        <img class="cover-title" src="/images/head_images/park_map.png" alt="">
      </div>
    </section>
                  ...
                  <div class="col-10">
                    ...
                    <div v-show="mapMode === 'illustrated'">
                      ...
                      <p class="map-hint">
                        ...
                        <span
                            style="color:#e65100;">⚠️ 此圖為手繪示意圖，建築位置僅供參考，精確定位請切換「精準定位」。</span>
                      </p>
                    </div>
    ...
    <transition name="sheet">
      <div v-if="selectedLandmark && !calibMode" class="landmark-sheet" @click.stop>
        ...
        <div class="d-flex align-items-center justify-content-center" style="gap:12px; margin-top:8px;">
          <span class="landmark-tag" :class="'landmark-tag--' + selectedLandmark.tag">{{
              selectedLandmark.tagLabel
            }}</span>
      ...
      <div v-if="calibMode" class="calib-banner" @click.stop>
        ...
        <div v-if="calibRecords.length" class="calib-recorded-list">
          <div v-for="r in calibRecords" :key="r.id" class="calib-recorded-item">
            <span>{{ r.name }}</span>
            <span class="calib-recorded-gps">{{ r.lat.toFixed(5) }}, {{ r.lng.toFixed(5) }} ±{{
    ...
    <transition name="sheet">
      <div v-if="showCalibOutput && !calibMode" class="calib-output-panel" @click.stop>
        ...
        <div class="calib-output-actions">
          ...
          <button class="map-ctrl-btn" style="color:#c62828;border-color:#c62828;" @click="clearCalibRecords">清除記錄
          </button>
        </div>
      </div>
    </transition>
