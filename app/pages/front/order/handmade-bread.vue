  ...
  <div class="hb-page">
    ...
    <div class="hb-wrap">
      ...
      <div v-if="!customer" class="hb-notice hb-notice--info">
        <svg xmlns="http://www.w3.org/2000/svg" class="hb-notice__icon" viewBox="0 0 24 24" fill="none"
             stroke="currentColor" stroke-width="2">
          <path d="M9 11l3 3L22 4"/>
          <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/>
        </svg>
      ...
      <div class="hb-day-tabs">
        <button v-for="opt in pickupDayOptions" :key="opt.dateKey"
          ...
          <span v-if="closedMap[opt.dateKey]" class="hb-day-tab__closed">休息日</span>
        </button>
      </div>
      <button v-if="weeksShown < MAX_WEEKS_SHOWN" type="button" class="hb-show-more-btn" @click="showMoreWeeks">
        顯示更多取貨日 ↓
      </button>
      ...
      <div class="hb-package">
        ...
        <button type="button" class="hb-package__btn" @click="applyMonthPackage">加入整月取貨日（套用以上品項數量）
        </button>
      </div>
        ...
        <div class="hb-field">
          ...
          <div class="hb-field__suggest-wrap">
            ...
            <div v-if="showSuggest" class="hb-suggest">
              <div v-for="n in suggestions" :key="n" class="hb-suggest__item" @mousedown.prevent="pickName(n)">{{
                  n
                }}
              </div>
            </div>
      ...
      <Transition name="hb-err-fade">
        <div v-if="errorMsg" class="hb-error">
          <svg xmlns="http://www.w3.org/2000/svg" class="hb-error__icon" viewBox="0 0 24 24" fill="none"
               stroke="currentColor" stroke-width="2">
            <circle cx="12" cy="12" r="10"/>
            <line x1="12" y1="8" x2="12" y2="12"/>
            <line x1="12" y1="16" x2="12.01" y2="16"/>
      ...
      <button class="hb-submit" :disabled="submitting || selDates.length === 0 || selDates.some(dk => isDateClosed(dk))"
              @click="doSubmit">
          ...
          <div class="hb-modal hb-modal--success">
            ...
            <div class="hb-modal__btns">
              <button class="confirm"
                      @click="customer ? $router.push('/front/profile/log?tab=handmade-bread') : $router.push('/')">
...
.hb-login-panel__avatar {
  width: 36px;
  height: 36px;
  border-radius: 50%;
}

.hb-login-panel__name {
  font-size: 13.5px;
  font-weight: 600;
  color: #2a2e25;
  margin: 0;
...
.hb-panel-fade-enter-active, .hb-panel-fade-leave-active {
  transition: opacity 0.15s, transform 0.15s;
}

.hb-panel-fade-enter-from, .hb-panel-fade-leave-to {
  opacity: 0;
...
.hb-day-tab.active .hb-day-tab__date {
  color: #fff;
}
...
.hb-suggest__item:last-child {
  border-bottom: none;
}

.hb-suggest__item:hover {
  background: #fdf4ea;
}
...
.hb-summary__row--day:first-child {
  padding-top: 0;
}
...
.hb-error__close:hover {
  opacity: 1;
}

.hb-err-fade-enter-active, .hb-err-fade-leave-active {
  transition: opacity 0.2s, transform 0.2s;
}

.hb-err-fade-enter-from, .hb-err-fade-leave-to {
  opacity: 0;
...
.hb-modal__btns button.confirm:hover {
  background: #5c381f;
}

/* ── Transitions ── */
.hb-modal-fade-enter-active, .hb-modal-fade-leave-active {
