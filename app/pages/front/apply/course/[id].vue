...
// 新報名（還沒有 myRegistration，answers 是空的）時，如果欄位標題看起來是姓名／
// 電話／Email／地址／生日這種基本資料，就直接用登入者的既有資料帶入，減少要重複
// 手動輸入已經登入時系統就知道的東西；已經報名過、正在編輯的資料一律以原本填的
// 答案為準，不會被這個覆蓋掉（resetAnswers 裡 existing 的優先權比這個高）
const guessDefaultAnswer = (f) => {
  if (!customer.value) return ''
  const label = f.label || ''
  if (f.type === 'text') {
    if (label.includes('姓名') || label.includes('名字')) return customer.value.name || ''
    if (label.includes('電話') || label.includes('手機')) return customer.value.mobile || customer.value.landline || ''
    if (label.includes('Email') || label.includes('email') || label.includes('信箱')) return customer.value.email || ''
    if (label.includes('地址')) return customer.value.address || ''
  }
  if (f.type === 'date' && label.includes('生日')) return customer.value.birthday || ''
  return ''
}
...
const resetAnswers = () => {
  answerFields.value.forEach(f => {
    ...
    answers[f.id] = existing ?? (f.type === 'checkbox' ? [] : guessDefaultAnswer(f))
    if (f.allowNote) {
        ...
        <div class="cr-layout">
          ...
          <span class="cr-badge cr-badge--main">
            ...
            <template v-else>
              ...
              <!-- 繳費資訊：直接顯示在頁面上可以編輯，不用另外跳 Modal -->
              <div v-if="course.paymentEnabled && course.myRegistration && !editing" class="cr-card">
                <div class="cr-card__title">
                ...
                <p v-if="course.paymentInfo?.trim()" class="cr-payment-box__info">{{ course.paymentInfo }}</p>
                <img
                    v-if="course.paymentInfoImage"
                    :src="imgUrl(course.paymentInfoImage)"
                    alt="收款資訊"
                    class="cr-payment-image"
                >
                <label class="cr-payment-card__label">繳費備註（選填，例如匯款後五碼）</label>
                <input
                    v-model="paymentNoteInput"
                    type="text"
                    placeholder="例如：後五碼 12345"
                    class="cr-note-input cr-payment-card__note"
                >
                <button
                    class="cr-submit"
      ...
      <Transition name="cr-modal-fade">
        <div v-if="successModal" class="cr-modal-backdrop" @click.self="successModal = false">
          <div class="cr-modal cr-modal--success">
            ...
            <!-- 繳費資訊改在頁面上的「繳費資訊」卡片直接編輯，這裡只提醒一下位置在哪 -->
            <p v-if="course?.paymentEnabled" class="cr-payment-hint">
              應繳 {{ course.myRegistration?.priceLabel || '' }}
              <template v-if="course.myRegistration?.amount">・${{ course.myRegistration.amount }}</template>
              ，請至下方「繳費資訊」完成匯款並填寫備註
            </p>
...
.cr-payment-box__auto {
  display: inline-block; font-size: 14px; font-weight: 600; color: #1a5c3a;
  background: #f0f9f4; border: 1px solid #b8d8c4; border-radius: 20px;
...
.cr-payment-image {
  ...
  border: 1px solid #dce8d8; margin: 8px 0;
}
.cr-payment-card__label { display: block; font-size: 13px; color: #5a6e54; margin: 0 0 6px; font-weight: 500; }
.cr-payment-card__note { margin-top: 0; margin-bottom: 10px; }
...
.cr-payment-hint { font-size: 11px; color: #8a9e84; margin: 0 0 1rem; line-height: 1.6; }
